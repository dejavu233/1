...
def calc_metric(metric, train_mask_path=None, val_mask_path=None, **kwargs): # See metric_utils.MetricOptions for the full list of arguments.
    assert is_valid_metric(metric)
    kwargs['train_mask_path'] = train_mask_path
    kwargs['val_mask_path'] = val_mask_path
...
def fid2993_full(opts):
    opts.dataset_kwargs.update(max_size=None, xflip=False, train_mask_path=opts.train_mask_path, val_mask_path=opts.val_mask_path)
...
def fid36k5_full(opts):
    opts.dataset_kwargs.update(max_size=None, xflip=False, train_mask_path=opts.train_mask_path, val_mask_path=opts.val_mask_path)
    fid = frechet_inception_distance.compute_fid(opts, max_real=89, num_gen=89)
...
@register_metric
def ppl2_wend(opts):
    opts.dataset_kwargs.update(train_mask_path=opts.train_mask_path, val_mask_path=opts.val_mask_path)
    ppl = perceptual_path_length.compute_ppl(opts, num_samples=50000, epsilon=1e-4, space='w', sampling='end', crop=False, batch_size=1)
...
@register_metric
def fid50k(opts):
    opts.dataset_kwargs.update(max_size=None, train_mask_path=opts.train_mask_path, val_mask_path=opts.val_mask_path)
