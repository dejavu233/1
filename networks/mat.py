...
from networks.basic_module import FullyConnectedLayer, Conv2dLayer, MappingNet, MinibatchStdLayer, DisFromRGB, DisBlock, \
    ...
    def __init__(
        ...
        if norm_cfg:
            if norm_cfg['type'] == 'BN':
                ...
                raise NotImplementedError(f"归一化层 '{norm_cfg['type']}' 未实现。")
        # 激活层
        if act_cfg:
            if act_cfg['type'] == 'ReLU':
                ...
                raise NotImplementedError(f"激活层 '{act_cfg['type']}' 未实现。")
...
class CAA(nn.Module):
    """上下文锚点注意力 (Context Anchor Attention) 模块。"""
...
class FirstStage(nn.Module):
    def __init__(self, img_channels, img_resolution=256, dim=180, w_dim=512, use_noise=False, demodulate=True,
        ...
        # --- CAA模块实例化 ---
        # 为每个Transformer阶段创建一个CAA模块
        self.caas = nn.ModuleList([CAA(channels=dim) for _ in range(len(depths))])
    ...
    def forward(self, images_in, masks_in, ws, noise_mode='random'):
        ...
        for i, block in enumerate(self.tran):  # 64 to 16
            if i < mid:
                ...
                # --- CAA模块应用点 (编码器部分) ---
                x_feature = token2feature(x, x_size)
                attention_map = self.caas[i](x_feature)
                x_attended = x_feature * attention_map
                x = feature2token(x_attended)
                # ------------------------------------
                ...
                ws_mapped = self.ws_style(ws[:, -1])
                add_n = self.to_square(ws_mapped).unsqueeze(1)
                ...
                style = torch.cat([gs, ws_mapped], dim=1)
