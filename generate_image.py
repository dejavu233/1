...
@click.command()
@click.pass_context
@click.option('--network', 'network_pkl', help='Network pickle filename', required=True, default=r"D:\MAT2\shuchu\00004-train-mirror-places512-mat-lr0.001-TwoStageLoss-pr0.1-nopl-kimg20-batch1-tc0.5-sm0.5-ema1-noaug-resumecustom\network-snapshot-000020.pkl")
@click.option('--dpath', help='the path of the input image', required=True, default=r"D:\train\cut")
@click.option('--mpath', help='the path of the mask', default=r"D:\AOT\datas\xinmask")
@click.option('--resolution', type=int, help='resolution of input image', default=512, show_default=True)
@click.option('--trunc', 'truncation_psi', type=float, help='Truncation psi', default=0.2, show_default=True)
@click.option('--noise-mode', help='Noise mode', type=click.Choice(['const', 'random', 'none']), default='const', show_default=True)
@click.option('--outdir', help='Where to save the output images', type=str, required=True, metavar='DIR', default=r"D:\MAT\定量比较\gaicaa")
def generate_images(
    ctx: click.Context,
    network_pkl: str,
    dpath: str,
