...
class ImageFolderMaskDataset(Dataset):
    def __init__(self,
        ...
        hole_range=[0,1],
        mask_path=None,         # Path to the directory containing masks
        ...
        **super_kwargs,         # Additional arguments for the Dataset base class.
        ...
        self._mask_path = mask_path  # Store the mask path
        self._train_mask_path = train_mask_path  # 存储训练集掩码路径
        self._val_mask_path = val_mask_path  # 存储验证集掩码路径
        ...
        if self._mask_path is not None:
            self._mask_fnames = sorted(glob.glob(os.path.join(self._mask_path, '*.png')) + glob.glob(os.path.join(self._mask_path, '*.jpg')))
            assert len(self._image_fnames) == len(self._mask_fnames), 'Number of images and masks must match'
    ...
    def __getitem__(self, idx):
        ...
        if self._mask_path is not None:
            mask = cv2.imread(self._mask_fnames[idx], cv2.IMREAD_GRAYSCALE).astype(np.float32) / 255.0
